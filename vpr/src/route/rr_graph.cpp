...
static std::vector<bool> alloc_and_load_perturb_opins(const t_type_ptr type, const vtr::Matrix<int>& Fc_out, const int max_chan_width, const int num_seg_types, const t_segment_inf* segment_inf);
...
static int alloc_and_load_rr_switch_inf(const int num_arch_switches, const float R_minW_nmos, const float R_minW_pmos, const int wire_to_arch_ipin_switch, int* wire_to_rr_ipin_switch);
...
static std::vector<vtr::Matrix<int>> alloc_and_load_actual_fc(const int L_num_types, const t_type_ptr types, const int max_pins, const int num_seg_types, const t_segment_inf* segment_inf, const int* sets_per_seg_type, const int max_chan_width, const e_fc_type fc_type, const enum e_directionality directionality, bool* Fc_clipped);
...
static int get_opin_direct_connecions(int x, int y, e_side side, int opin, int from_rr_node, t_rr_edge_info_set& rr_edges_to_create, const t_rr_node_indices& L_rr_node_indices, const std::vector<t_rr_node>& rr_nodes, const t_direct_inf* directs, const int num_directs, const t_clb_to_clb_directs* clb_to_clb_directs) {
    ...
    for (int i = 0; i < num_directs; i++) {
        ...
        if (clb_to_clb_directs[i].from_clb_type == curr_type) {  //We are at a valid starting point
            ...
            if (directs[i].from_side != NUM_SIDES && directs[i].from_side != side)
                continue;
