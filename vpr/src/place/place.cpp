...
static void initial_placement_location(int* free_locations, ClusterBlockId blk_id, int* pipos, int* px, int* py, int* pz);
...
static bool find_to(t_type_ptr type, float rlim, int x_from, int y_from, int* px_to, int* py_to, int* pz_to);
static void find_to_location(t_type_ptr type, float rlim, int x_from, int y_from, int* px_to, int* py_to, int* pz_to);
...
static void placement_inner_loop(float t, float rlim, t_placer_opts placer_opts, int move_lim, float crit_exponent, int inner_recompute_limit, t_placer_statistics* stats, t_placer_costs* costs, t_placer_prev_inverse_costs* prev_inverse_costs, int* moves_since_cost_recompute,
#ifdef ENABLE_CLASSIC_VPR_STA
    t_slack* slacks,
    t_timing_inf timing_inf,
#endif
...
static void initial_placement_location(int* free_locations, ClusterBlockId blk_id, int* pipos, int* px_to, int* py_to, int* pz_to) {
    auto& cluster_ctx = g_vpr_ctx.clustering();
