...
static void add_override_constraint(char** from_list, int num_from, char** to_list, int num_to, float constraint, int num_multicycles, bool domain_level_from, bool domain_level_to, bool make_copies);
...
static bool build_from_to_lists(char*** from_list, int* num_from, bool* domain_level_from, char*** to_list, int* num_to, bool* domain_level_to, const sdcparse::StringGroup& from_group, const sdcparse::StringGroup& to_group);
        ...
        for (sink_clock_domain = 0; sink_clock_domain < timing_ctx.sdc->num_constrained_clocks; sink_clock_domain++) {
            if ((icc = find_cc_constraint(timing_ctx.sdc->constrained_clocks[source_clock_domain].name, timing_ctx.sdc->constrained_clocks[sink_clock_domain].name)) != -1) {
                if (timing_ctx.sdc->cc_constraints[icc].num_multicycles == 0) {
                    ...
                    timing_ctx.sdc->domain_constraint[source_clock_domain][sink_clock_domain] = calculate_constraint(sdc_clocks[source_clock_domain], sdc_clocks[sink_clock_domain])
...
static float calculate_constraint(t_sdc_clock source_domain, t_sdc_clock sink_domain) {
    ...
    if (fabs(source_domain.period - sink_domain.period) < EPSILON && fabs(source_domain.rising_edge - sink_domain.rising_edge) < EPSILON && fabs(source_domain.falling_edge - sink_domain.falling_edge) < EPSILON) {
