...
void strongconnect(int& index, int* tnode_indexes, int* tnode_lowlinks, bool* tnode_instack, std::stack<int>& tnode_stack, std::vector<std::vector<int> >& tnode_sccs, size_t min_size, int inode);
...
void strongconnect(int& index, int* tnode_indexes, int* tnode_lowlinks, bool* tnode_instack, std::stack<int>& tnode_stack, std::vector<std::vector<int> >& tnode_sccs, size_t min_size, int inode) {
    ...
    for (iedge = 0; iedge < timing_ctx.tnodes[inode].num_edges; iedge++) {
        ...
        if (to_node_index == DO_NOT_ANALYSE)
            continue;  //Skip marked invalid nodes
