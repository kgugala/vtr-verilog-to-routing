...
static void update_slacks(t_slack* slacks, float criticality_denom, bool update_slack, bool is_final_analysis, float smallest_slack_in_design, const t_timing_inf& timing_inf);
...
static float do_timing_analysis_for_constraint(int source_clock_domain, int sink_clock_domain, bool is_prepacked, bool is_final_analysis, long* max_critical_input_paths_ptr, long* max_critical_output_paths_ptr, vtr::vector<ClusterBlockId, t_pb**>& pin_id_to_pb_mapping, const t_timing_inf& timing_inf);
...
static void load_tnode(t_pb_graph_pin* pb_graph_pin, const ClusterBlockId iblock, int* inode);

#ifndef PATH_COUNTING
static void update_normalized_costs(float T_arr_max_this_domain, long max_critical_input_paths, long max_critical_output_paths, const t_timing_inf& timing_inf);
#endif
...
t_slack* alloc_and_load_pre_packing_timing_graph(float inter_cluster_net_delay, t_timing_inf timing_inf, const std::unordered_map<AtomBlockId, t_pb_graph_node*>& expected_lowest_cost_pb_gnode) {
                ...
                if (has_valid_T_arr(inode)) {                  /* If this tnode has been used */
                    ...
                    if (num_edges == 0) { /* sink */
                        ...
                        for (iedge = 0; iedge < num_edges; iedge++) {
                            ...
                            if (to_node == DO_NOT_ANALYSE)
                                continue;  //Skip marked invalid nodes
        ...
        for (sink_clock_domain = 0; sink_clock_domain < timing_ctx.sdc->num_constrained_clocks; sink_clock_domain++) {
            if (!constraint_used[sink_clock_domain]) {
                if (timing_ctx.sdc->domain_constraint[source_clock_domain][sink_clock_domain] != DO_NOT_ANALYSE) {
                    VTR_LOG_WARN(
                        "Timing constraint from clock %d to %d of value %f will be disabled"
                    ...
                    for (int i = 0; i < num_at_level; i++) {
                        ...
                        if (num_edges == 0) { /* sink */
                            ...
                            for (int iedge = 0; iedge < num_edges; iedge++) {
                                ...
                                if (timing_ctx.tnodes[to_node].num_edges == 0 && timing_ctx.tnodes[to_node].clock_domain == sink_clock_domain) {  // one away from a register on this sink domain
...
static float do_timing_analysis_for_constraint(int source_clock_domain, int sink_clock_domain, bool is_prepacked, bool is_final_analysis, long* max_critical_input_paths_ptr, long* max_critical_output_paths_ptr, vtr::vector<ClusterBlockId, t_pb**>& pin_id_to_pb_mapping, const t_timing_inf& timing_inf) {
    ...
    for (ilevel = timing_ctx.num_tnode_levels - 1; ilevel >= 0; ilevel--) {
        ...
        for (i = 0; i < num_at_level; i++) {
            ...
            if (ilevel == 0) {
                if (!(timing_ctx.tnodes[inode].type == TN_INPAD_SOURCE || timing_ctx.tnodes[inode].type == TN_FF_SOURCE || timing_ctx.tnodes[inode].type == TN_CONSTANT_GEN_SOURCE || timing_ctx.tnodes[inode].type == TN_CLOCK_SOURCE) && !timing_ctx.tnodes[inode].is_comb_loop_breakpoint) {
            ...
            if (num_edges == 0) { /* sink */
                ...
                f_timing_stats->cpd[source_clock_domain][sink_clock_domain] = max(f_timing_stats->cpd[source_clock_domain][sink_clock_domain],
                    (timing_ctx.tnodes[inode].T_arr - timing_ctx.tnodes[inode].clock_delay));
                ...
                for (iedge = 0; iedge < num_edges; iedge++) {
                    ...
                    if (timing_ctx.tnodes[to_node].num_edges == 0 && timing_ctx.tnodes[to_node].clock_domain == sink_clock_domain) {
                        f_timing_stats->least_slack[source_clock_domain][sink_clock_domain] = min(f_timing_stats->least_slack[source_clock_domain][sink_clock_domain],
        ...
        for (iedge = 0; iedge < num_edges; iedge++) {
            ...
            if (timing_inf.slack_definition != std::string("S") && timing_inf.slack_definition != std::string("G")) {
                if (!is_final_analysis) {  // Criticality is not meaningful using non-normalized slacks.
                    ...
                    slacks->timing_criticality[inet][iedge + 1] = max(timing_criticality, slacks->timing_criticality[inet][iedge + 1]);
                    ...
                    slacks->path_criticality[inet][iedge + 1] = max(slacks->path_criticality[inet][iedge + 1],
                        (timing_ctx.tnodes[inode].forward_weight + timing_ctx.tnodes[to_node].backward_weight) * pow((float)FINAL_DISCOUNT_FUNCTION_BASE, timing_criticality - 1));
                    ...
                    slacks->path_criticality[inet][iedge + 1] = max(slacks->path_criticality[inet][iedge + 1],
                        log(timing_ctx.tnodes[inode].forward_weight * timing_ctx.tnodes[to_node].backward_weight) * pow((float)FINAL_DISCOUNT_FUNCTION_BASE, timing_criticality - 1));
#elif PATH_COUNTING == 'R' /* Use product of natural logs of forward and backward weights. */
                    slacks->path_criticality[inet][iedge + 1] = max(slacks->path_criticality[inet][iedge + 1],
                        log(timing_ctx.tnodes[inode].forward_weight) * log(timing_ctx.tnodes[to_node].backward_weight) * pow((float)FINAL_DISCOUNT_FUNCTION_BASE, timing_criticality - 1));
#endif
#endif
                }
            }
...
#ifndef PATH_COUNTING
static void update_normalized_costs(float criticality_denom, long max_critical_input_paths, long max_critical_output_paths, const t_timing_inf& timing_inf) {
    int inode;
    ...
    for (inode = 0; inode < timing_ctx.num_tnodes; inode++) {
        ...
        if (has_valid_T_arr(inode) && has_valid_T_req(inode)) {
            ...
            timing_ctx.tnodes[inode].prepacked_data->normalized_total_critical_paths = max(timing_ctx.tnodes[inode].prepacked_data->normalized_total_critical_paths,
                ((float)timing_ctx.tnodes[inode].prepacked_data->num_critical_input_paths + timing_ctx.tnodes[inode].prepacked_data->num_critical_output_paths) / ((float)max_critical_input_paths + max_critical_output_paths));
        }
    }
}
#endif
...
static const char* find_tnode_net_name(int inode, bool is_prepacked, vtr::vector<ClusterBlockId, t_pb**>& pin_id_to_pb_mapping) {
    ...
    if (is_prepacked) {
        ...
        if (timing_ctx.tnodes[inode].type == TN_INPAD_SOURCE || timing_ctx.tnodes[inode].type == TN_INPAD_OPIN || timing_ctx.tnodes[inode].type == TN_OUTPAD_SINK || timing_ctx.tnodes[inode].type == TN_OUTPAD_IPIN) {
            AtomBlockId blk_id = atom_ctx.nlist.pin_block(pin_id);
...
float get_critical_path_delay() {
    ...
    if (!timing_ctx.sdc)
        return UNDEFINED; /* If timing analysis is off, for instance. */
...
vtr::vector<ClusterBlockId, std::vector<int>> alloc_and_load_tnode_lookup_from_pin_id() {
    ...
    for (int i = 0; i < timing_ctx.num_tnodes; i++) {
        if (timing_ctx.tnodes[i].pb_graph_pin != nullptr) {
            if (timing_ctx.tnodes[i].type != TN_CLOCK_SOURCE && timing_ctx.tnodes[i].type != TN_FF_SOURCE && timing_ctx.tnodes[i].type != TN_INPAD_SOURCE && timing_ctx.tnodes[i].type != TN_FF_SINK && timing_ctx.tnodes[i].type != TN_OUTPAD_SINK) {
