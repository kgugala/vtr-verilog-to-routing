...
class PrintCallback : public tp::Callback {
    ...
    void add_node(int node_id, tp::NodeType type, std::vector<int> in_edge_ids, std::vector<int> out_edge_ids) override {
        ...
        fprintf(stdout, "  type:");
        switch (type) {
            case tp::NodeType::SOURCE:
                fprintf(stdout, " SOURCE");
                break;
            case tp::NodeType::SINK:
                fprintf(stdout, " SINK");
                break;
            case tp::NodeType::IPIN:
                fprintf(stdout, " IPIN");
                break;
            case tp::NodeType::OPIN:
                fprintf(stdout, " OPIN");
                break;
            default:
                assert(false);
    ...
    void add_edge(int edge_id, tp::EdgeType type, int src_node_id, int sink_node_id, bool disabled) override {
        ...
        fprintf(stdout, "  type:");
        switch (type) {
            case tp::EdgeType::PRIMITIVE_COMBINATIONAL:
                fprintf(stdout, " PRIMITIVE_COMBINATIONAL");
                break;
            case tp::EdgeType::PRIMITIVE_CLOCK_LAUNCH:
                fprintf(stdout, " PRIMITIVE_CLOCK_LAUNCH");
                break;
            case tp::EdgeType::PRIMITIVE_CLOCK_CAPTURE:
                fprintf(stdout, " PRIMITIVE_CLOCK_CAPTURE");
                break;
            case tp::EdgeType::INTERCONNECT:
    ...
    void add_node_tag(tp::TagType type, int node_id, int launch_domain_id, int capture_domain_id, float time) override {
        fprintf(stdout, " type: ");
        switch (type) {
            case tp::TagType::SETUP_LAUNCH_CLOCK:
                fprintf(stdout, "SETUP_LAUNCH_CLOCK");
                break;
            case tp::TagType::SETUP_CAPTURE_CLOCK:
                fprintf(stdout, "SETUP_CAPTURE_CLOCK");
                break;
            case tp::TagType::HOLD_LAUNCH_CLOCK:
                fprintf(stdout, "HOLD_LAUNCH_CLOCK");
                break;
            case tp::TagType::HOLD_CAPTURE_CLOCK:
                fprintf(stdout, "HOLD_CAPTURE_CLOCK");
                break;
            default:
                assert(false);
        }
    ...
    void add_edge_slack(tp::TagType type, int edge_id, int launch_domain_id, int capture_domain_id, float slack) override {
        fprintf(stdout, " type: ");
        switch (type) {
            case tp::TagType::SETUP_SLACK:
                fprintf(stdout, "SETUP_SLACK");
                break;
            case tp::TagType::HOLD_SLACK:
                fprintf(stdout, "HOLD_SLACK");
                break;
            default:
                assert(false);
        }
